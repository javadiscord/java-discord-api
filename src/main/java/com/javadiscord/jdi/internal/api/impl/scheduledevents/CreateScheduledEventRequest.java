...
public record CreateScheduledEventRequest(long guildId, Optional<Long> channelId,
        Optional<EntityMetadata> entityMetadata, String name, int privacyLevel, // 2 is only option
                                                                                // atm
        @JsonFormat(shape = JsonFormat.Shape.STRING,
                pattern = "yyyy-MM-dd'T'HH:mm:ss.SSSXXX") OffsetDateTime scheduledStartTime,
        @JsonFormat(shape = JsonFormat.Shape.STRING,
                pattern = "yyyy-MM-dd'T'HH:mm:ss.SSSXXX") Optional<OffsetDateTime> scheduledEndTime,
        Optional<String> description, int entityType, Optional<String> image)
